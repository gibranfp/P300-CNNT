...
#
"""
Script to evaluate the OCLNN architecture for single-trial subject-dependent P300 detection 
"""
import argparse
import sys
import numpy as np
...
def evaluate_subject_model(X_train, y_train, X_valid, y_valid, X_test, y_test, timepath):
    ...
    segment_size = int(np.ceil(n_samples / 15))
    padding = 15 - (n_samples % 15)
    if padding == 15:
        padding = 0
    elif padding % 2 == 0:
        padding = int(padding / 2)
    else:
        padding = (int(np.ceil(padding / 2)), int(np.floor(padding / 2)))
    print('OCLNN Segment Size = {0}, Padding = {1}'.format(segment_size, padding))

    model = OCLNN(Chans = n_channels, Samples = n_samples, SegmentSize = segment_size, Padding = padding)
    ...
    df.to_csv(timepath + 'OCLNN_times.csv', encoding='utf-8')
