...
#
"""
Script to evaluate the DeepConvNet architecture (Lawhern et al., 2018) for single-trial subject-dependent P300 detection 
"""
import argparse
import sys
import numpy as np
...
def evaluate_subject_models(data, labels, modelpath, subject):
    ...
    for k, (t, v) in enumerate(cv.split(data[subject], labels[subject])):
        ...
        model = DeepConvNet(2, Chans = n_channels, Samples = n_samples)
        print(model.summary())
