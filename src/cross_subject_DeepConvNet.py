...
#
"""
Script to evaluate the DeepConvNet architecture for single-trial cross-subject P300 detection
"""
import argparse
import sys
import numpy as np
...
def evaluate_cross_subject_model(data, labels, modelpath):
    ...
    for k, (t, v) in enumerate(cv.split(data, labels, groups)):
        ...
        model = DeepConvNet(2, dropoutRate = 0.25, Chans = n_channels, Samples = n_samples)
        print(model.summary())
