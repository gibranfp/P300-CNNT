...
#
"""
Script to evaluate the OCLNN architecture (Shan et al., 2018) for single-trial subject-dependent P300 detection 
"""
import argparse
import sys
import numpy as np
...
def evaluate_subject_models(data, labels, modelpath, subject):
    """
    Trains and evaluates OCLNN for each subject in the P300 Speller database
    ...
    for k, (t, v) in enumerate(cv.split(data[subject], labels[subject])):
        ...
        model = OCLNN(Chans = n_channels, Samples = n_samples)
        print(model.summary())
