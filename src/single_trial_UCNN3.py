...
#
"""
Script to evaluate a modified CNN5A3 architecture (Cecotti et al., 2011) for single-trial subject-dependent P300 detection 
"""
import argparse
import sys
import numpy as np
...
def evaluate_subject_models(data, labels, modelpath, subject):
    """
    Trains and evaluates a modified CNN3 for each subject in the P300 Speller database
    ...
    for k, (t, v) in enumerate(cv.split(data[subject], labels[subject])):
        ...
        model = UCNN3(Chans = 6, Samples = 206)
        print(model.summary())
