...
#
"""
Script to evaluate a modified CNN1 architecture (Lawhern et al., 2018) for single-trial subject-dependent P300 detection 
"""
import argparse
import sys
import numpy as np
from CNN1 import UCNN1
...
def evaluate_subject_models(data, labels, modelpath, subject):
    """
    Trains and evaluates the modified CNN1 for each subject in the P300 Speller database
    ...
    for k, (t, v) in enumerate(cv.split(data[subject], labels[subject])):
        ...
        model = UCNN1(Chans = 6, Samples = 206)
        print(model.summary())
